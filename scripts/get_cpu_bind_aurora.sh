...
if [ "${#args[@]}" -lt 1 ] || [ "${#args[@]}" -gt 2 ]; then
    echo "Usage: $0 <ranks_per_node> [shift_amount] [--logical]" >&2
    exit 1
fi
...
if [[ ! ${ranks_per_node} =~ ^[1-9][0-9]*$ ]]; then
    echo "ranks_per_node must be a positive integer (got ${ranks_per_node})" >&2
    exit 1
fi
if [[ ! ${shift_amount} =~ ^[0-9]+$ ]]; then
    echo "shift_amount must be a non-negative integer (got ${shift_amount})" >&2
    exit 1
fi

max_ranks=$((sockets * usable_physical_per_socket))
if [ "$include_logical" -eq 1 ]; then
    max_ranks=$((max_ranks * 2))
fi
if [ "$ranks_per_node" -gt "$max_ranks" ]; then
    echo "ranks_per_node exceeds the ${max_ranks}-rank topology limit" >&2
    exit 1
fi
...
if [ "$ranks_per_node" -eq 1 ]; then
    if [ "$shift_amount" -ne 0 ]; then
        echo "shift_amount must be zero when ranks_per_node is 1" >&2
        exit 1
    fi
...
if [ "$ranks_per_socket" -le "$usable_physical_per_socket" ]; then
    ...
    max_shift=$((usable_physical_per_socket - cores_per_rank * ranks_per_socket))
else
    max_shift=0
fi

if [ "$shift_amount" -gt "$max_shift" ]; then
    echo "shift_amount ${shift_amount} exceeds the maximum ${max_shift} for ${ranks_per_node} ranks" >&2
    exit 1
fi
