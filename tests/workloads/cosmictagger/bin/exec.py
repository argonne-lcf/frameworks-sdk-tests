...
''')
    ...
    def add_io_arguments(self, parser):
        packaged_data = pathlib.Path(network_dir) / "example_data"
        data_directory = pathlib.Path(
            os.environ.get("COSMICTAGGER_DATA_DIR", str(packaged_data))
        )
        default_input = data_directory / "cosmic_tagging_light.h5"
