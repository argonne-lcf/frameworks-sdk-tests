...
class tf_trainer(trainercore):
    ...
    def init_network(self):
        ...
        if self.args.precision == "bfloat16":
            self.policy = tf.keras.mixed_precision.Policy('mixed_bfloat16')
            tf.keras.mixed_precision.set_global_policy(self.policy)
        ...
        # Build variables with a bounded dummy forward pass.  Network-only tests do
        # not initialize an input queue, and consuming training data here would also
        # discard the first real minibatch.
        image, label = self.cast_input(
            numpy.zeros(batch_dims, dtype=numpy.float32),
            numpy.zeros(batch_dims, dtype=numpy.int32),
        )
    ...
    def n_parameters(self):
        n_trainable_parameters = 0
        for var in self._net.trainable_variables:
            n_trainable_parameters += numpy.prod(var.get_shape())
    ...
    def forward_pass(self, image, label, training):
        ...
        prediction = [
            tf.argmax(x, axis=self._channels_dim, output_type=tf.dtypes.int32)
            for x in logits
        ]
    ...
    def apply_gradients(self, gradients):
        pairs = [
            (gradient, variable)
            for gradient, variable in zip(gradients, self._net.trainable_variables)
    ...
    def gradient_step(self, image, label):
        ...
        #
            if self.args.precision == "mixed":
                if hasattr(self._opt, "scale_loss"):
        ...
        if self.args.precision == "mixed":
            scaled_gradients = self.get_gradients(
                scaled_loss, tape, self._net.trainable_weights
            )
            if hasattr(self._opt, "scale_loss"):
                # Keras 3 LossScaleOptimizer unscales in apply_gradients().
    ...
    def train_step(self):
        ...
        for i in range(self.args.gradient_accumulation):
            ...
            if gradients is None:
                ...
                gradients = [
                    new if accumulated is None else accumulated
                    if new is None else accumulated + new
                    for accumulated, new in zip(gradients, internal_gradients)
                ]
        ...
        if self.args.gradient_accumulation != 1:
            gradients = [
                None if gradient is None else gradient / self.args.gradient_accumulation
