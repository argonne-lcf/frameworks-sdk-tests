...
class Block(tf.keras.layers.Layer):
    ...
    def __init__(self, *,
        ...
        self.convolution = tf.keras.layers.Conv2D(
            ...
            kernel_regularizer  = tf.keras.regularizers.L2(l2=params.weight_decay)
        )
...
class ResidualBlock(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training):
        ...
        x = self.convolution_1(x, training=training)

        x = self.convolution_2(x, training=training)
...
class DeepestBlock(tf.keras.layers.Layer):
    ...
    def call(self, x, training):
        ...
        if not self.block_concat:
            ...
            x = self.unbottleneck(x, training=training)

            x = tf.split(x, 3, self.channels_axis)

        else:
            x = [ self.bottleneck(_x, training=training) for _x in x ]
            x = [ self.blocks(_x, training=training) for _x in x ]
            x = [ self.unbottleneck(_x, training=training) for _x in x ]
        return x
...
class MaxPooling(tf.keras.models.Model):
    ...
    def call(self, x, training):
        ...
        return self.bottleneck(x, training=training)
...
class UNetCore(tf.keras.models.Model):
    ...
    def call(self, x, training):
        ...
        if self._depth_of_network != 0:
            ...
            x = [ self.down_blocks(_x, training=training) for _x in x ]
            ...
            x = [ self.downsample(_x, training=training) for _x in x ]
        ...
        x = self.main_module(x, training=training)
...
class UResNet(tf.keras.models.Model):
    ...
    def call_internal(self, input_tensor, training):
        ...
        x = [ self.initial_convolution(_x, training=training) for _x in x ]


        # Apply the main unet architecture:
        x = self.net_core(x, training=training)

        # Apply the final residual block to each plane:
        if self.final_blocks:
            x = [ self.final_layer(_x, training=training) for _x in x ]
