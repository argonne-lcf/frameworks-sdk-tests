...
config_lit.amplitude_limit = 100
config_lit.conf = 0.0
...
params_lit_init, nparams_lit = wavefunction_lit_aux.build()
...
if (config.module_lit_load):
    with open(model_lit_save_path, 'rb') as file:
        ...
        params_lit = jax.tree_util.tree_map(wavefunction_lit_aux.update_mix, params_lit_init, params_lit_read)
else:
    params_lit = params_lit_init
...
def LIT_optimize(r_d, sz_d, params_lit, d_overlap, m_overlap, g2_overlap, sig_R, sig_I, key_batch):
    params_lit = jax.tree_util.tree_map(wavefunction_lit_aux.update_mix, params_lit_init, params_lit)
    overlap_list = []
...
#        neuralit.shard_map_adam_overlap(it, params_lit, params_gs, r_l_batch, sz_l_batch, r_d_batch, sz_d_batch, m_overlap, g2_overlap, sig_R, sig_I)
        overlap, overlap_lhs, overlap_rhs, overlap_test, d_overlap, m_overlap, g2_overlap, ess = \
...
n_sig_R = 11
sig_R_list = jnp.linspace(-10, 10, n_sig_R)
