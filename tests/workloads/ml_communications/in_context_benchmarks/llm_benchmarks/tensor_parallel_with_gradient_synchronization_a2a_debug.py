...
parser.add_argument(
    ...
    dest="master_addr",
    default=None,
    help="Rendezvous hostname (defaults to rank 0's hostname)",
)
...
shared_comm.Free()
visible_device = local_rank % get_device_count(args.device)
set_device(visible_device)
device = torch.device(args.device, visible_device)
...
if rank == 0:
   master_addr              = args.master_addr or socket.gethostname()
   logging.info(f"Master Address = {master_addr}")
   master_port              = int(os.environ.get("MASTER_PORT", "2345"))
else:
   master_addr              = None
   master_port              = None
